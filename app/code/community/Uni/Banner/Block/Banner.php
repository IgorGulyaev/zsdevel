...
class Uni_Banner_Block_Banner extends Mage_Core_Block_Template {
    ...
    protected function _construct()
    {
         ...
         */
        if (!$this->getTemplate()) {
            $this->setTemplate('unibanner/banner.phtml');
        }
    }
    ...
    {
        return Mage::getModel('banner/bannergroup')
            ->getDataByGroupCode($groupCode);
    }

    public function getResizeImage($bannerPath, $groupName, $w = 0, $h = 0) {
        $name = null;
        $bannerDirPath = Mage::helper('banner')
            ->updateDirSepereator($bannerPath);
